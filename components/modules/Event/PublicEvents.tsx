...
export default function PublicEvents({
  ...
  useEffect(() => {
    ...
    if (selectedStatus !== "Status") params.set("status", selectedStatus);

    if (selectedFee === "Free") params.set("maxFee", "0");
    if (selectedFee === "Paid") params.set("minFee", "1");

    params.set("page", "1");
    params.set("limit", "5");

    router.push(`${pathname}?${params.toString()}`);
  }, [searchQuery, selectedType, selectedFee,selectedStatus]);
  ...
  return (
    <div
      className={`w-full ${pathname === "/events" ? "mt-24" : ""}`}
    >
      <div className="container mx-auto px-8 pb-16">
        <EventsHeader
          searchQuery={searchQuery}
